...
Visit [Docker Cloud](https://cloud.docker.com/swarm/gogs/repository/docker/nameisalreadytakenexception/gogs) / [Docker Store](https://store.docker.com/community/images/nameisalreadytakenexception/gogs) see all available images and tags.
...
$ docker pull nameisalreadytakenexception/gogs

# Create local directory for volume.
$ mkdir -p /var/gogs

# Use `docker run` for the first time.
$ docker run --name=gogs -p 10022:22 -p 10080:3000 -v /var/gogs:/data nameisalreadytakenexception/gogs
...
The "custom" directory may not be obvious in Docker environment. The `/var/nameisalreadytakenexception/gogs` (in the host) and `/data/gogs` (in the container) is already the "custom" directory and you do not need to create another layer but directly edit corresponding files under this directory.
...
docker run --name=gogs-data --entrypoint /bin/true nameisalreadytakenexception/gogs

# Use `docker run` for the first time.
docker run --name=gogs --volumes-from gogs-data -p 10022:22 -p 10080:3000 nameisalreadytakenexception/gogs
```
...
# Use `docker run` for the first time.
$ docker run --name=gogs -p 10022:22 -p 10080:3000 -v gogs-data:/data nameisalreadytakenexception/gogs
```
